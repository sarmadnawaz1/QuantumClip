...
import time
import uuid
from utils import load_config, normalize_image_bytes_to_size
    ...
    def generate_image(self, prompt, width=1408, height=768, steps=4):
        ...
        try:
            ...
            if isinstance(output, list) and len(output) > 0:
                ...
                # Normalize to 1704x960 for consistency across app
                return normalize_image_bytes_to_size(response.content, 1704, 960)
            else:
    ...
    def generate_image(self, prompt, width=1408, height=768, steps=4, model=None, timeout=DEFAULT_TIMEOUT, max_retries=DEFAULT_MAX_RETRIES):
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Normalize to 1704x960 for consistency across app
                return normalize_image_bytes_to_size(image_data, 1704, 960)
        ...
        for attempt in range(max_retries):
            try:
                ...
                if result:
                    ...
                    # Normalize to 1704x960 for consistency across app
                    return normalize_image_bytes_to_size(result, 1704, 960)
                else:
        ...
        try:
            ...
            if "data" in result and len(result["data"]) > 0:
                ...
                # If no base64 data, check for image URL and download it
                    ...
                    # Normalize to 1704x960 for consistency across app (Runware generates 1344x768)
                    return normalize_image_bytes_to_size(image_data, 1704, 960)
