...
class PromptService:
    ...
    def generate_image_prompt(
        ...
        normalized_style = image_style.lower().replace(" ", "").replace("-", "").replace("_", "")
        if normalized_style in {"filmnoir", "monochrome"}:
            final_prompt = self._enforce_black_and_white(final_prompt)
        print(f"🎨 [PROMPT] Final prompt generated with style '{image_style}': {final_prompt[:100]}...")
        return final_prompt

    def _enforce_black_and_white(self, prompt: str) -> str:
        """Ensure monochrome styles stay strictly grayscale."""
        color_words = [
            "red", "blue", "green", "yellow", "orange", "purple", "pink", "teal",
            "cyan", "magenta", "maroon", "indigo", "violet", "crimson", "scarlet",
            "gold", "silver", "bronze", "azure", "emerald", "sapphire", "ruby",
            "turquoise", "lavender", "beige", "brown"
        ]
        pattern = r"\b(" + "|".join(color_words) + r")\b"
        prompt = re.sub(pattern, "gray", prompt, flags=re.IGNORECASE)
        ...
        if not any(tag in prompt.lower() for tag in enforcement_tags):
            prompt += ", strictly black and white monochrome, grayscale only, zero color"
        return prompt
    ...
    def _get_style_description(self, image_style: str) -> str:
        ...
        try:
            from app.utils import get_custom_styles
        ...
        elif style_normalized == "film_noir" or "film noir" in style_lower:
            return (
                "Cinematic film noir still frame, strictly black and white photographic palette with zero color, deep shadows, harsh chiaroscuro lighting, "
                "overhead street lamps cutting through rainy night, cigarette smoke, venetian blind shadows across faces, trench coats and fedoras, "
                "moody 1940s detective drama, dramatic highlights, high contrast monochrome aesthetic. NO COLOR. BLACK AND WHITE ONLY."
            )
        elif "monochrome" in style_lower:
            return (
                "Black and white monochrome photography with rich grayscale tones, dramatic contrast, elegant lighting, "
                "artistic composition, timeless noir cinematography, absence of color, classic vintage look."
            )
        elif "lofi" in style_lower or "lofiaesthetic" in style_lower:
            return (
                "Lo-fi anime aesthetic: cozy bedroom interior, warm pastel lighting, retro technology, cassette players, "
                "vinyl records, window with rain, soft focus, warm muted tones, nostalgic atmosphere, studying vibe, "
                "calming, chill ambience."
            )
